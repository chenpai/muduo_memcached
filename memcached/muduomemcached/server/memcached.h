...
typedef struct {
    ...
    muduo::net::Channel notify_event;
...
struct conn {
    ...
    item   *item2;  //item -> item2   /* for commands set/add/replace  */
...
conn *conn_new(const int sfd, const enum conn_states init_state, const int event_flags, const int read_buffer_size, enum network_transport transport, muduo::net::EventLoop *base);
