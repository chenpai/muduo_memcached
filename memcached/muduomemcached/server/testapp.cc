...
static pid_t start_server(in_port_t *port_out, bool daemon, int timeout) {
    ...
    */
    ...
    if (pid == 0) {
        ...
        if (!daemon) {
			s = "./timedrun";
            argv[arg++] = const_cast<char *>(s.data());
            argv[arg++] = tmo;
        }
		s = "./memcached-debug";
        argv[arg++] = const_cast<char *>(s.data());
...
static enum test_return test_issue_44(void) {
    in_port_t port2;
    pid_t pid = start_server(&port2, true, 15);
...
static struct addrinfo *lookuphost(const char *hostname, in_port_t port2)
{
    ...
    (void)snprintf(service, NI_MAXSERV, "%d", port2);
...
static int connect_server(const char *hostname, in_port_t port2, bool nonblock)
{
    struct addrinfo *ai = lookuphost(hostname, port2);
    int sock2 = -1;
    if (ai != NULL) {
       if ((sock2 = socket(ai->ai_family, ai->ai_socktype,
                          ai->ai_protocol)) != -1) {
          if (connect(sock2, ai->ai_addr, ai->ai_addrlen) == -1) {
             ...
             sock2 = -1;
              ...
              if (flags < 0 || fcntl(sock2, F_SETFL, flags | O_NONBLOCK) < 0) {
...
static void read_ascii_response(char *buffer, size_t size) {
    ...
    do {
        ...
        if (nr == -1) {
            ...
            offset += nr;
            assert((unsigned int)(offset + 1) < size);
        }
    } while (need_more);
}
...
static off_t flush_command(char* buf, size_t bufsz, uint8_t cmd, uint32_t exptime, bool use_extra) {
    protocol_binary_request_flush *request = (protocol_binary_request_flush *)buf;
...
static off_t touch_command(char* buf,
                           ...
                           size_t keylen,
                           uint32_t exptime) {
    protocol_binary_request_touch *request = (protocol_binary_request_touch *)buf;
    assert(bufsz > sizeof(*request));
...
static off_t arithmetic_command(char* buf,
                                ...
                                uint64_t initial,
                                uint32_t exp) {
    protocol_binary_request_incr *request = (protocol_binary_request_incr *)buf;
...
static enum test_return test_binary_decr_impl(const char* key, uint8_t cmd) {
    ...
    for (ii = 9; ii >= 0; --ii) {
        ...
        if (cmd == PROTOCOL_BINARY_CMD_DECREMENT) {
            ...
            validate_response_header(&receive.response_header, cmd,
                                     PROTOCOL_BINARY_RESPONSE_SUCCESS);
            assert((int)ntohll(receive.response.message.body.value) == ii);
        }
    }
