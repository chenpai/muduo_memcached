...
static int max_fds;
static muduo::net::EventLoop *main_base;
...
static void maxconns_handler(const int fd, const short which, void *arg) {
    ...
    if (fd == -42 || allow_new_conns == false) {
		...
		main_base->runAfter(10, boost::bind(maxconns_handler,fd,which,arg));
    } else {
...
static void *conn_timeout_thread(void *arg) {
    ...
    while(1) {
        ...
        for (i = 0; i < max_fds; i++) {
            ...
            if ((current_time - c->last_cmd_time) > (unsigned int)settings.idle_timeout) {
                buf[0] = 't';
...
static const char *prot_text(enum protocol prot) {
	...
	char *rv = const_cast<char *>(s.data());
    return rv;
}

void conn_close_idle(conn *c) {
    if (settings.idle_timeout > 0 &&
        (current_time - c->last_cmd_time) > (unsigned int)settings.idle_timeout) {
        if (c->state != conn_new_cmd && c->state != conn_read) {
...
void conn_worker_readd(conn *c) {
    ...
    c->event.channelSet(c->thread->base,c->sfd);  
	c->event.setReadCallback(boost::bind(event_handler,c->sfd,c->which, (void *)c));
...
conn *conn_new(const int sfd, enum conn_states init_state,
	...
	c->event.channelSet(base,sfd);  
	c->event.setReadCallback(boost::bind(event_handler,c->sfd,c->which, (void *)c));
	if(event_flags & c->event.getkReadEvent())
		c->event.enableReading();
	else
		c->event.disableReading();
	if(event_flags & c->event.getkWriteEvent())
		c->event.enableWriting();
	else
		c->event.disableWriting();
	if(event_flags == c->event.getkNoneEvent())
	    c->event.disableAll();
...
static void complete_nread_ascii(conn *c) {
    ...
    item_remove(c->item2);       /* release the c->item reference */
    c->item2 = 0;
}
...
static void add_bin_header(conn *c, uint16_t err, uint8_t hdr_len, uint16_t key_len, uint32_t body_len) {
    ...
    if (add_msghdr(c) != 0) {
         ...
         */
		char s[] = "SERVER_ERROR out of memory adding binary header";
        out_of_memory(c, s);
        return;
    }
...
static void complete_incr_bin(conn *c) {
    ...
    case EOM:
		{
		char s[] = "SERVER_ERROR Out of memory incrementing value";
        out_of_memory(c, s);
        break;
		}
    case DELTA_ITEM_NOT_FOUND:
        if (req->message.body.expiration != 0xffffffff) {
            ...
            if (it != NULL) {
				...
				char s[] = "SERVER_ERROR Out of memory allocating new item";
                out_of_memory(c,s);
            }
        } else {
            pthread_mutex_lock(&c->thread->stats.mutex);
...
static void append_bin_stats(const char *key, const uint16_t klen,
    ...
    protocol_binary_response_header header = {
        ...
        header.response.datatype = (uint8_t)PROTOCOL_BINARY_RAW_BYTES,
        header.response.bodylen = (uint8_t)htonl(bodylen),
        header.response.opaque = (uint8_t)c->opaque
    };
...
static void process_bin_stat(conn *c) {
    ...
    if (nkey == 0) {
        ...
        if (get_stats(subcommand, nkey, &append_stats, c)) {
            if (c->stats.buffer == NULL) {
				char s[] = "SERVER_ERROR Out of memory generating stats";
                out_of_memory(c, s);
            } else {
...
static void process_bin_complete_sasl_auth(conn *c) {
    ...
    char mech[nkey+1];
    memcpy(mech, ITEM_key((item*)c->item2), nkey);
    mech[nkey] = 0x00;

    if (settings.verbose)
        fprintf(stderr, "mech:  ``%s'' with %d bytes of data\n", mech, vlen);

    const char *challenge = vlen == 0 ? NULL : ITEM_data((item*) c->item2);

    if (vlen > ((item*) c->item2)->nbytes) {
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_EINVAL, NULL, vlen);
        c->write_and_go = conn_swallow;
        item_unlink(c->item2);
        return;
    ...
    case SASL_OK:
		...
		char s[] = "Authenticated";
        write_bin_response(c, s, 0, 0, strlen("Authenticated"));
        pthread_mutex_lock(&c->thread->stats.mutex);
    ...
    switch (c->cmd) {
        case PROTOCOL_BINARY_CMD_VERSION:
            if (extlen == 0 && keylen == 0 && bodylen == 0) {
				char s[] = VERSION;
                write_bin_response(c, s, 0, 0, strlen(VERSION));
            } else {
                protocol_error = 1;
            }
            break;
...
static void process_bin_update(conn *c) {
    ...
    if (it == 0) {
        ...
        if (! item_size_ok(nkey, req->message.body.flags, vlen + 2)) {
			...
			char s[] = "SERVER_ERROR Out of memory allocating item";
            out_of_memory(c, s);
...
static void write_and_free(conn *c, char *buf, int bytes) {
    if (buf) {
		...
		char s[] = "SERVER_ERROR out of memory writing stats";
        out_of_memory(c, s);
    }
}
...
inline static void process_stats_detail(conn *c, const char *command) {
    ...
    if (strcmp(command, "on") == 0) {
        ...
        int len;
        char *stats2 = stats_prefix_dump(&len);
        write_and_free(c, stats2, len);
    }
    else {
...
static void conn_to_str(const conn *c, char *buf) {
    ...
    if (!c) {
        ...
        struct sockaddr *addr = (sockaddr *)(void *)&c->request_addr;
        int af;
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
        ...
        while(key_token->length != 0) {
            ...
            if (it) {
                ...
                if (return_cas || !settings.inline_ascii_response)
                {
                  ...
                  suffix = (char *)do_cache_alloc(c->thread->suffix_cache);
                  if (suffix == NULL) {
                      STATS_LOCK();
                      stats.malloc_fails++;
                      STATS_UNLOCK();
					  char s[]= "SERVER_ERROR out of memory making CAS suffix";
                      out_of_memory(c, s);
                      item_remove(it);
                      while (i-- > 0) {
                          item_remove(*(c->ilist + i));
    ...
    */
    if (key_token->value != NULL || add_iov(c, "END\r\n", 5) != 0
		...
		char s[] = "SERVER_ERROR out of memory writing get response";
        out_of_memory(c, s);
    }
    else {
        conn_set_state(c, conn_mwrite);
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    if (it == 0) {
        ...
        if (! item_size_ok(nkey, flags, vlen)) {
			...
			char s[] = "SERVER_ERROR out of memory storing object";
            out_of_memory(c, s);
            //status = NO_MEMORY;
        }
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const bool incr) {
    ...
    case EOM:
		{
		char s[] = "SERVER_ERROR out of memory";
        out_of_memory(c, s);
        break;
		}
    case DELTA_ITEM_NOT_FOUND:
...
static void process_command(conn *c, char *command) {
    ...
    c->iovused = 0;
    if (add_msghdr(c) != 0) {
		char s[] = "SERVER_ERROR out of memory preparing response";
        out_of_memory(c, s);
        return;
    }
...
static int try_read_command(conn *c) {
    ...
    if (c->protocol == binary_prot) {
        ...
        if ((unsigned int)c->rbytes < sizeof(c->binary_header)) {
            /* need more data! */
            return 0;
        } else {
#ifdef NEED_ALIGN
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            if (!new_rbuf) {
				...
				char s[] = "SERVER_ERROR out of memory reading request";
                out_of_memory(c, s);
                c->write_and_go = conn_closing;
...
static bool update_event(conn *c, const int new_flags) {
    ...
    muduo::net::EventLoop *base = c->event.ownerLoop();
	...
	c->event.disableAll();
	c->event.remove();
	c->event.channelSet(base,c->sfd);  
	c->event.setReadCallback(boost::bind(event_handler,c->sfd,c->which, (void *)c));
	if(new_flags & c->event.getkReadEvent())
		c->event.enableReading();
	else
		c->event.disableReading();
	if(new_flags & c->event.getkWriteEvent())
		c->event.enableWriting();
	else
		c->event.disableWriting();
	if(new_flags == c->event.getkNoneEvent())
	    c->event.disableAll();
...
void do_accept_new_conns(const bool do_accept) {
    ...
    for (next = listen_conn; next; next = next->next) {
        if (do_accept) {
            update_event(next, listen_conn->event.getkReadEvent());
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
        ...
        if (res > 0) {
            ...
            while (m->msg_iovlen > 0 && res >= (unsigned int)m->msg_iov->iov_len) {
        ...
        if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            if (!update_event(c, c->event.getkWriteEvent())) {
                if (settings.verbose > 0)
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        case conn_listening:
            ...
            if (settings.maxconns_fast &&
                stats_state.curr_conns + stats_state.reserved_fds >= (unsigned int)(settings.maxconns - 1)) {
                ...
                dispatch_conn_new(sfd, conn_new_cmd, c->event.getkReadEvent(),
        ...
        case conn_waiting:
            if (!update_event(c, c->event.getkReadEvent())) {
                if (settings.verbose > 0)
                    fprintf(stderr, "Couldn't update event\n");
                conn_set_state(c, conn_closing);
                break;
        ...
        case conn_nread:
            ...
            if (!c->item2 || (((item *)c->item2)->it_flags & ITEM_CHUNKED) == 0) {
                /* first check if we have leftovers in the conn_read buffer */
                if (c->rbytes > 0) {
                    int tocopy = c->rbytes > c->rlbytes ? c->rlbytes : c->rbytes;
                    if (c->ritem != c->rcurr) {
        ...
        if (IS_UDP(transport)) {
            ...
            for (c = 0; c < settings.num_threads_per_udp; c++) {
                ...
                dispatch_conn_new(per_thread_fd, conn_read,
                                  listen_conn_add->event.getkReadEvent(),
                                  UDP_READ_BUFFER_SIZE, transport);
            }
        } else {
            if (!(listen_conn_add = conn_new(sfd, conn_listening,
                                             listen_conn_add->event.getkReadEvent(), 1,
                                             transport, main_base))) {
...
static int server_socket_unix(const char *path, int access_mask) {
    ...
    if (!(listen_conn = conn_new(sfd, conn_listening,
                                 listen_conn->event.getkReadEvent(), 1,
...
static void clock_handler(const int fd, const short which, void *arg) {
	...
	main_base->runAfter(1000, boost::bind(clock_handler,fd,which,arg));
...
static bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
    ...
    for (p = strtok_r(s, "-", &b);
         p != NULL;
         p = strtok_r(NULL, "-", &b)) {
        if (!safe_strtoul(p, &size) || size < (unsigned int)settings.chunk_size
             || size > (unsigned int)settings.slab_chunk_size_max) {
...
int main (int argc, char **argv) {
    LOG_INFO << "muduo-memcached begin!";
	int c;
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'I':
            ...
            if ((unsigned int)settings.item_size_max > (settings.maxbytes / 4)) {
       ...
       case 'X' :
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case NO_INLINE_ASCII_RESP:
                ...
                break;
            case MODERN:
                // Modernized defaults. Need to add equivalent no_* flags before making truly default. 
...
//    }
     ...
     */
    usleep(1000);
    if (stats_state.curr_conns + stats_state.reserved_fds >= (unsigned int)(settings.maxconns - 1)) {
